<header class="slider slider-prlx">
    <div class="swiper-container parallax-slider">
        <div class="swiper-wrapper">
            <div class="swiper-slide">
                <div class="bg-img valign" data-background="/teenus/assets/img/slider-2.jpg" data-overlay-dark="3">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-8 col-md-12">
                                <div class="caption">
                                    <h1>¿Cada vez tus clientes son menos? </h1>
                                    <!-- <h2>¡No permitas que tu precio espante a tus clientes!</h2> -->
                                    <h5 class="mb-3"><!-- Déjame contarte que existe una solución…<br><br> -->
                                        <a href="/tezliksoftware" style="color:yellow">TezlikSoftware<span></span></a> te ayuda a realizar estrategias de precios, negociación de Materias Primas y a vender con tus descuentos para ganar mucho más 😁. 🔵 No vas a necesitar de conocimientos técnicos ni contables.😊. ¡Contáctanos Ya!
                                    </h5>
                                    <div class="banner-btn home-slider-btn">
                                        <a href="/tezliksoftware" class="default-btn-one">Conoce más<span></span></a>
                                        <a class="default-btn" href="#contact">Contáctanos Ya<span></span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="bg-img valign" data-background="/teenus/assets/img/slider-1.jpg" data-overlay-dark="3">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-8 col-md-12">
                                <div class="caption">
                                    <h1>¿Quieres vender más?</h1> <br>
                                    <!-- <h2>¡El problema no es tu producto es el precio!</h2> -->
                                    <h5 class="mb-3"><!-- Déjame contarte que existe una solución...<br><br> --><a href="/tezlik" style="color:yellow">TezlikSoftware <span></span></a>es una solución de bajo costo que tiene más de 8 años en la industria y que puede ayudarte a fijar los precios y crear estrategias para vender más 😊. 🔵 No necesitas de conocimientos técnicos ni de contables. 😁 ¡Contáctanos Ya! </h5>
                                    <div class="banner-btn home-slider-btn">
                                        <a href="/tezliksoftware" class="default-btn-one">Conoce más<span></span></a>
                                        <a class="default-btn" href="#contact">Contactanos Ya<span></span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="bg-img valign" data-background="/teenus/assets/img/slider-3.jpg" data-overlay-dark="3">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-8 col-md-12">
                                <div class="caption">
                                    <h1>Aprende cuanto cobrar por tus productos</h1> <br>
                                    <!-- <h2>desde casa o empresa y sin necesidad de tener conocimientos contables, potencializa tu negocio para hacerlo altamente rentable en menos de 10 días </h2> -->
                                    <h5 class="mb-3"><!-- Déjame contarte que existe una solución...<br><br> --><a href="/tezlik" style="color:yellow">TezlikSoftware <span></span></a>es una solución de bajo costo que te ayuda a costear tus productos para asegurar que la empresa sea rentable a largo plazo y pueda continuar operando de manera sostenible. 😊. 🔵 No necesitas de conocimientos técnicos ni de contables. 😁 ¡Contáctanos Ya! </h5>
                                    <div class="banner-btn home-slider-btn">
                                        <a href="/tezliksoftware" class="default-btn-one">Conoce más<span></span></a>
                                        <a class="default-btn" href="#contact">Contactanos Ya<span></span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- slider setting -->
        <div class="control-text">
            <div class="swiper-button-prev swiper-nav-ctrl prev-ctrl cursor-pointer">
                <span class="arrow prv"></span>
            </div>
            <div class="swiper-button-next swiper-nav-ctrl next-ctrl cursor-pointer">
                <span class="arrow nxt"></span>
            </div>
        </div>
        <div class="swiper-pagination"></div>
    </div>
</header>